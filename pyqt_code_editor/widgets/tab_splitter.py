...
class TabSplitter(QWidget):
    def __init__(self, parent=None, initial_path=None):
        super().__init__(parent)
        logger.info("TabSplitter initialized")
        ...
        self.initial_tab = TabbedEditor(self)
        self.initial_tab.lastTabClosed.connect(self.avoid_empty_splitter)
        self.initial_tab.add_code_editor(initial_path)
    ...
    def avoid_empty_splitter(self):
        logger.info('Avoiding empty splitter')
        self.initial_tab.add_code_editor()

    def create_tabbed_editor(self, initial_path=None):
        """Utility to create a TabbedEditor and connect lastTabClosed so we can remove splits."""
        t = TabbedEditor(self)
        if initial_path is not None:
            t.add_code_editor(initial_path)
    ...
    def select_and_open_file(self):
        ...
        if not active_tab_widget:
            logger.info("No focused TabbedEditor found; cannot open file.")
            return        
    ...
    def split(self, orientation):
        ...
        # The parent is probably the main QSplitter or another splitter
        ...
        if not isinstance(parent_splitter, QSplitter):
            logger.info("The focused TabbedEditor is not inside a QSplitter. Cannot split further.")
            return
        ...
        logger.info("Creating a new QSplitter with orientation '%s'",
                    "Horizontal" if orientation == Qt.Horizontal else "Vertical")
        new_splitter = QSplitter(orientation, self)
    ...
    def active_tab_widget(self):
        ...
        # look through all children of TabSplitter and return the first QTabWidget you find.
    ...
    def handle_tabbed_editor_empty(self, emptied_editor):
        ...
        if not isinstance(parent_splitter, QSplitter):
            logger.info("The emptied TabbedEditor has no QSplitter parent, ignoring.")
            return
    ...
    def _flatten_upwards(self, splitter):
        ...
        for i in range(splitter.count()):
            ...
            if isinstance(w, QWidget) and not isinstance(w, QSplitter) and not isinstance(w, TabbedEditor):
                w.setParent(None)
                w.deleteLater()
        ...
        if splitter == self.central_splitter:
            ...
            if isinstance(single_child, QSplitter):
                ...
                logger.info("Single child is not a QSplitter. No further flattening.")
            return
        else:
    ...
    def _remove_splitter_from_parent(self, splitter, single_child=None):
        ...
        if not isinstance(parent_splitter, QSplitter):
            logger.info("Parent of this splitter is not another QSplitter. Possibly main window. Doing nothing.")
            return
